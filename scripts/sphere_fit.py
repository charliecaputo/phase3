...
class SphereFitNode:
    def __init__(self):
        ...
        # Time variables for pause feature
        self.start_time = time.time()
        self.pause_duration = 10  # 10 seconds
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            if time.time() - self.start_time >= self.pause_duration:
                rospy.loginfo("Pause after 10 seconds")
                time.sleep(30)  # Pause for 30 seconds
                self.start_time = time.time()  # Reset the start time after pause
